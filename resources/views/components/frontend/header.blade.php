        ...
        <div class="flex justify-between items-center h-20">
                    ...
                    @foreach ($menuCategories as $key => $menu)
                        ...
                        @if ($menu->submenus->isNotEmpty())
                            ...
                            <div class="relative group">
                                <a href="#" id="categories-btn"
                                    class="text-gray-900 hover:text-theme-color-hover inline-flex items-center focus:outline-none">
                                    {{ $menu->name }}
                                ...
                                <div class="w-80 z-10 hidden absolute flex items-end">
                                    <div class=" hidden group-hover:block bg-white shadow-lg py-2 mt-2 rounded-md w-48 z-50 h-auto">
                                        ...
                                        <a href="{{ $submenu->link }}"
                                            class="block px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-theme-color-hover">{{
                                            $submenu->name }}</a>
                                        @endforeach
                                    </div>
                                </div>
            ...
            <div class="flex items-center space-x-4">
                ...
                <button class="text-gray-900 hover:text-indigo-600 flex items-center justify-center">
                    <img src="{{ asset('images/icons/icon_search.png') }}" alt="Search"
                        class="h-6 w-6 object-contain" />
                </button>

                <!-- Cart Icon -->
                <button class="text-gray-900 hover:text-indigo-600 flex items-center justify-center pb-1">
                    <img src="{{ asset('images/icons/icon_cart_header.png') }}" alt="Cart"
                        class="h-6 w-6 object-contain" />
                </button>
                ...
                <div class="relative">
                    <button id="user-btn"
                        class="text-gray-900 hover:text-indigo-600 focus:outline-none flex items-center justify-center">
                        <img src="{{ asset('images/icons/user.png') }}" alt="User" class="h-6 w-6 object-contain" />
                    </button>
                    <!-- Login/Registration Dropdown -->
                    <div id="user-dropdown"
                        class="hidden absolute right-0 bg-white shadow-lg py-2 mt-2 rounded-md w-48 z-50">
        ...
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            @if( $menuCategories )
                @foreach ($menuCategories as $key => $menu)
                    ...
                    <div>
                        <button
                            class="w-full text-left text-gray-900 hover:text-indigo-600 flex justify-between items-center py-2"
                            onclick="toggleCollapse('category-1')">
                            {{ $menu->name }}
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 transform transition-transform duration-300"
                                id="arrow-category-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                            </svg>
                        </button>
                        <div id="category-1" class="hidden pl-4">
                            @foreach ($menu->submenus as $submenu)
                            <a href="#" class="block text-gray-700 hover:bg-gray-100 py-1">{{ $submenu->name }}</a>
                            @endforeach
                        </div>
                    </div>
                    @else
                        <a href="#" class="block text-gray-900 hover:text-indigo-600">{{ $menu->name }}</a>
                    @endif
                @endforeach
            @endif
        </div>
...
<script>
    ...
    function closeDropdowns(exceptId) {
        ...
        dropdowns.forEach((id) => {
            ...
            if (element && id !== exceptId) {  // Check if element exists
